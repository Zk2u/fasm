...
use fasm_storage::{
    Commit, KvPair, KvStore, KvStream, RetryableStorageError, bound_to_owned, is_empty_range,
    next_prefix,
};
...
fn end_selector(prefix: &[u8], bound: Bound<Vec<u8>>) -> KeySelector<'static> {
    match bound {
        ...
        Bound::Unbounded => {
            // `[0xFF]` is FoundationDB's end-of-keyspace key (developer guide:
            // a range to `\xFF` extends to the end of the database). It is a
            // safe sentinel: keys beginning with 0xFF are system-reserved,
            // unreadable and unwritable without the ACCESS_SYSTEM_KEYS
            // transaction option, which this crate never sets, so the
            // sentinel cannot exclude a key a handle of this crate can hold.
...
fn clear_range_end_key(prefix: &[u8], bound: Bound<Vec<u8>>) -> Vec<u8> {
    match bound {
        ...
        // Same end-of-keyspace sentinel as `end_selector`.
...
impl KvStore for FdbTransaction {
    ...
    fn range<'a>(
        ...
        if is_empty_range(&start, &end) {
            // Answered locally; no FDB cursor is opened for a range that
            // names no keys.
    ...
    async fn clear_range(
        ...
        if is_empty_range(&start, &end) {
            // A clear that names no keys is a no-op. An empty or inverted
            // bound pair must not reach FDB: an inverted range raises
            // `inverted_range` into the transaction's deferred error, which
            // the next operation on the transaction re-raises.
            return Ok(());
        }
        let start = bound_to_owned(start);
        let end = bound_to_owned(end);
